...
def clean_prods(p_order):
    ...
    for entry in order:
        names.append(entry[0][1][0])
        flavours.append(entry[0][1][1])
        size.append(entry[1][1])
        price.append(entry[2][1])
...
def clean_transactions(
    date, name, store, total, method, clean_date_time=clean_date_time
):
    trans_dict = {
        "date_time": date,
        "customer": name,
...
def main_clean(payload):
    ...
    card, customer, store = quick_cards_cust_store(
        payload["card_number"], payload["customer_name"], payload["store"]
    )
