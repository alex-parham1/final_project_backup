...
print('USE EXTREME CAUTION RUNNING THIS SCRIPT. SNOWFLAKE DOES NOT ENFORCE DUPLICATES. BE CERTAIN THAT THE DATA YOU ARE QUERYING IS WHAT YOU WANT TO GO INTO SNOWFLAKE.')
...
while migrate:
    table_name_rds = input('Enter the exact name of the RDS table you wish to copy *CASE SENSITIVE*:')
    table_name_snowflake = input('Enter the exact name of the SNOWFLAKE table you wish to copy into *CASE SENSITIVE*:')
    ...
    print(rds_df.head())
    go = input('Verify the above columns match what you expect in SNOWFLAKE. Press Y to continue or N to abort. > ').upper()
    if go == 'Y':
        connect_and_push_snowflake(table_name_snowflake, db_name_snowflake, rds_df, user=snow_user, password=snow_password)
        print('Success')
    else:
        pass
